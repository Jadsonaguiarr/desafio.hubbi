...
var client_default = prismaClient;
...
var ProdutoController = class {
  async create(req, res) {
    ...
    try {
      const schema = import_zod.z.object({
        ...
        preco: import_zod.z.number({
          required_error: "Adicione o pre\xE7o do produto!"
        })
      });
      const validateName = await client_default.produto.findUnique({
      ...
      schema.parse(data);
      await client_default.produto.create({
        data: {
          nome: data.nome,
          preco: data.preco
        }
  ...
  async getById(req, res) {
    try {
      ...
      const produto = await client_default.produto.findFirst({
        where: { id }
      });
      if (!produto) {
        res.status(404).json({
  ...
  async getall(req, res) {
    try {
      const produtos = await client_default.produto.findMany();
...
var ProdutoController_default = new ProdutoController();

// src/routes/produto.routes.ts
var path = "/produto";
var router = (0, import_express.Router)();
router.post(`${path}/`, ProdutoController_default.create);
router.get(`${path}/getall`, ProdutoController_default.getall);
router.get(`${path}/:id`, ProdutoController_default.getById);
var produto_routes_default = router;
...
var UserController = class {
  async create(req, res) {
    ...
    try {
      const schema = import_zod2.z.object({
        ...
        admin: import_zod2.z.coerce.boolean(),
        password: import_zod2.z.string({
          required_error: "Adicione uma senha!"
        })
      });
      schema.parse(data);
      const hashPassword = await import_bcryptjs.default.hash(data.password, 10);
  ...
  async getById(req, res) {
    try {
      const id = req.params.id;
      const user = await client_default.user.findFirst({ where: { id } });
      if (!user) {
        res.json({
          message: "Usuario n\xE3o encontrado!"
        });
      }
      res.json(user);
    } catch (error) {
...
router2.post(`${path2}/`, UserController_default.create);
router2.get(`${path2}/:id`, UserController_default.getById);
var user_routes_default = router2;

// src/routes/venda.routes.ts
var import_express3 = require("express");

// src/controllers/VendaControler.ts
...
var VendaController = class {
  async create(req, res) {
    try {
      ...
      const schema = import_zod3.z.object({
        produtos: import_zod3.z.array(
          import_zod3.z.object({
            id: import_zod3.z.string(),
            qtd: import_zod3.z.number(),
            preco: import_zod3.z.number()
          }),
          {
      ...
      const venda = await client_default.venda.create({
        data: {
          id_comprador: id,
          status: "Em preparo",
          total
      ...
      produtos.map(async (produto) => {
        const produto_venda = await client_default.produto_Venda.create({
          data: {
  ...
  async getById(req, res) {
    try {
      const id = req.params.id;
      const venda = await client_default.venda.findFirst({
        where: { id },
        include: {
          Produto_Venda: {
            include: {
              produto: true
              // Inclui todos os detalhes do produto associado
            }
          }
        }
      });
  ...
  async verificaVenda(id) {
    const venda = await client_default.venda.findFirst({ where: { id } });
    if (!venda) return;
    const produtosVenda = await client_default.produto_Venda.findMany({
    ...
    if (produtosVerificados) {
      const venda2 = await client_default.venda.update({
        where: { id },
        data: {
          status: "Finalizado"
        },
  ...
  async getAll(req, res) {
    try {
      const vendas = await client_default.venda.findMany({
  ...
  async totalDeVendas(req, res) {
    ...
    const vendasMesAtual = await client_default.venda.count({
      where: {
        createdAt: {
          gte: inicioMesAtual,
          lt: fimMesAtual
    ...
    const vendasMesAnterior = await client_default.venda.count({
      where: {
        createdAt: {
    ...
    res.status(200).json({ total: vendasMesAtual, percentual });
  }
};
var VendaControler_default = new VendaController();

// src/routes/venda.routes.ts
var path3 = "/venda";
var router3 = (0, import_express3.Router)();
router3.get(`${path3}/total`, VendaControler_default.totalDeVendas);
router3.get(`${path3}/getall`, VendaControler_default.getAll);
router3.get(`${path3}/:id`, VendaControler_default.getById);
router3.get(`${path3}/getbyuser/:id`, VendaControler_default.getAllByIdUser);
router3.post(`${path3}/:id`, VendaControler_default.create);
var venda_routes_default = router3;

// src/routes/compra.routes.ts
var import_express4 = require("express");

// src/controllers/CompraController.ts
var import_zod4 = require("zod");
var CompraController = class {
  async create(req, res) {
    const data = req.body;
    try {
      const schema = import_zod4.z.object({
        id_venda: import_zod4.z.string({
          required_error: "Erro ao escolher venda"
        }),
        produtos: import_zod4.z.array(
          import_zod4.z.object({
            id: import_zod4.z.string(),
            id_produto: import_zod4.z.string()
          }),
          {
            required_error: "Adicione itens na compra!"
          }
      ...
      if (!venda) {
        res.status(400).json({ message: "Venda n\xE3o encontrada!" });
      }
      const compra = await client_default.compra.create({
        data: {
      ...
      const produtos_venda = produtos.map(async (produto) => {
        const produtoAlterado = await client_default.produto_Venda.update({
          where: { id: produto.id },
          data: {
            status: "Comprado"
          }
        });
        await client_default.compra_Produto.create({
          data: {
      ...
      const response = await VendaControler_default.verificaVenda(data.id_venda);
  ...
  async getAll(req, res) {
    const compras = await client_default.compra.findMany({
      include: { Compra_Produto: { include: { produto: true } } }
    });
    res.status(200).json(compras);
  }
};
var CompraController_default = new CompraController();

// src/routes/compra.routes.ts
var path4 = "/comprar";
var router4 = (0, import_express4.Router)();
router4.post(`${path4}/`, CompraController_default.create);
router4.get(`${path4}/getall`, CompraController_default.getAll);
var compra_routes_default = router4;
...
// src/controllers/Authenticate.ts
...
var Authenticate = class {
  async login(req, res) {
    ...
    const user = await client_default.user.findFirst({ where: { cpf } });
    if (!user) {
      res.status(404).json({ message: "Usuario n\xE3o encontrado" });
      return;
    }
    const autenticada = await (0, import_bcryptjs2.compare)(password, user.password);
    if (!autenticada) {
      res.status(400).json({ message: "Senha incorreta!" });
      return;
    }
    const { expiresIn, secret } = config_default.jwt;
...
var Authenticate_default = new Authenticate();

// src/routes/authenticate.routes.ts
var path5 = "/auth";
var router5 = (0, import_express5.Router)();
router5.post(`${path5}/`, Authenticate_default.login);
var authenticate_routes_default = router5;

// src/routes/index.routes.ts
var app_router = (0, import_express6.default)();
app_router.use(produto_routes_default);
app_router.use(user_routes_default);
app_router.use(venda_routes_default);
app_router.use(compra_routes_default);
app_router.use(authenticate_routes_default);
var index_routes_default = app_router;

// src/server.ts
var app = (0, import_express7.default)();
var port = 4e3;
app.use((0, import_cors.default)({ credentials: true, origin: "http://localhost:3000" }));
app.use(import_express7.default.json());
app.use(import_body_parser.default.json());
app.use(index_routes_default);
app.listen(port, () => {
